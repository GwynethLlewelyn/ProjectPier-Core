    ...
    */
    function execute() {
      ...
      foreach ($tables_to_check as $table) {
        ...
        if (mysqli_num_rows(mysqli_execute(_query($this->database_connection, $test_table_exists_sql)))) {
      ...
      $rows = mysqli_query($this->database_connection, "SELECT * FROM `$users_table`");
      while ($row = mysqli_fetch_assoc($rows)) {
        if (!mysqli_query($this->database_connection, "INSERT INTO $contacts_table (`company_id` , `user_id` , `email` , `display_name` , `title` , `avatar_file` , `office_number` , `fax_number` , `mobile_number` , `home_number` , `created_on` , `created_by_id` , `updated_on`) VALUES ('".$row['company_id']."',  '".$row['id']."', '".$row['email']."', '".$row['display_name']."', '".$row['title']."', '".$row['avatar_file']."', '".$row['office_number']."', '".$row['fax_number']."' , '".$row['mobile_number']."' , '".$row['home_number']."' , '".$row['created_on']."', '".$row['created_by_id']."', '".$row['updated_on']."');")) {

          $this->printMessage("Error while creating contact. Operation aborted. MySQL said: ".mysqli_error($this->database_connection), true);
          mysqli_query($this->database_connection, 'ROLLBACK');
          return false;
        }
        $contacts_created++;
      ...
      $rows = mysqli_query($this->database_connection, "SELECT `$contacts_table`.`id`, `$contact_im_table`.`contact_id`, `$contact_im_table`.`im_type_id` FROM `$contacts_table`, `$contact_im_table` WHERE `$contacts_table`.`user_id` = `$contact_im_table`.`contact_id`");
      while ($row = mysqli_fetch_assoc($rows)) {
        if (!mysqli_query($this->database_connection, "UPDATE `$contact_im_table` SET `contact_id` = '".$row['id']."' WHERE `contact_id` = '".$row['contact_id']."' AND `im_type_id` = '".$row['im_type_id']."'")) {
          $this->printMessage("Error while updating Contact-IM table. Upgrade aborted. MySQL said: ".mysqli_error($this->database_connection), true);
          mysqli_query($this->database_connection, 'ROLLBACK');
          return false;
        } // if
